...
#define HTU21D_FIRMWARE_READ1 \
  0x84  // read firmware revision, 1-st part of the command
#define HTU21D_FIRMWARE_READ2 \
  0xB8  // read firmware revision, 2-nd part of the command

#define HTU21D_FIRMWARE_V1 0xFF  // sensor firmware v1.0
#define HTU21D_FIRMWARE_V2 0x20  // sensor firmware 2.0

#define HTU21D_TEMP_COEFFICIENT \
  -0.15  // temperature coefficient for RH compensation at range 0°C..80°C, for
         // HTU21D & SHT21 only
#define HTU21D_CRC8_POLYNOMINAL \
  0x13100  // crc8 polynomial for 16bit value, CRC8 -> x^8 + x^5 + x^4 + 1

#define HTU21D_SOFT_RESET_DELAY 15  // in milliseconds

#define HTU21D_FORCE_READ_TEMP \
  0xFE  // force to read temperature, see
        // https://github.com/enjoyneering/HTU21D/pull/3
#define HTU21D_ERROR \
  0xFF  // returns 255, if CRC8 or communication error is occurred

typedef enum : uint8_t {
  HTU21D_RES_RH12_TEMP14 =
      0x00,  // resolution, temperature: 14-Bit & humidity: 12-Bit
  HTU21D_RES_RH8_TEMP12 =
      0x01,  // resolution, temperature: 12-Bit & humidity: 8-Bit
  HTU21D_RES_RH10_TEMP13 =
      0x80,  // resolution, temperature: 13-Bit & humidity: 10-Bit
  HTU21D_RES_RH11_TEMP11 =
      0x81  // resolution, temperature: 11-Bit & humidity: 11-Bit
} HTU21D_RESOLUTION;

typedef enum : uint8_t {
  HTU21D_TRIGGER_HUMD_MEASURE_HOLD =
      0xE5,  // humidity measurement with hold master
  HTU21D_TRIGGER_HUMD_MEASURE_NOHOLD =
      0xF5  // temperature measurement with no hold master
} HTU21D_HUMD_OPERATION_MODE;

typedef enum : uint8_t {
  HTU21D_TRIGGER_TEMP_MEASURE_HOLD =
      0xE3,  // temperature measurement with hold master
  HTU21D_TRIGGER_TEMP_MEASURE_NOHOLD =
      0xF3,  // temperature measurement with no hold master
  SI70xx_TEMP_READ_AFTER_RH_MEASURMENT =
      0xE0  // read temperature value from previous RH measurement, for Si7021
            // only
...
class HTU21D {
 public:
  ...
  float readHumidity(
      HTU21D_HUMD_OPERATION_MODE =
          HTU21D_TRIGGER_HUMD_MEASURE_HOLD);  // max accuracy ±2%RH  in range
                                              // 20%..80% at 25°C
  float readCompensatedHumidity(
      float temperature =
          HTU21D_FORCE_READ_TEMP);  // max accuracy ±2%RH  in range 0%..100% at
                                    // 0°C..80°C
  float readTemperature(
      HTU21D_TEMP_OPERATION_MODE =
          HTU21D_TRIGGER_TEMP_MEASURE_HOLD);  // max accuracy ±0.3°C in range
                                              // 0°C..60°C

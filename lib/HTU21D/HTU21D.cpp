/***************************************************************************************************/
/*
  This is an Arduino library for SHT21, HTU21D & Si70xx Digital Humidity and
  Temperature Sensor written by : enjoyneering79 sourse code:
  https://github.com/enjoyneering/ This chip uses I2C bus to communicate,
  specials pins are required to interface Board: SDA                    SCL
  Level Uno, Mini, Pro, ATmega168, ATmega328..... A4                     A5 5v
  Mega2560................................. 20                     21 5v Due,
  SAM3X8E............................. 20 21                     3.3v Leonardo,
  Micro, ATmega32U4.............. 2                      3 5v Digistump,
  Trinket, ATtiny85............. 0/physical pin no.5    2/physical pin no.7 5v
  Blue Pill, STM32F103xxxx boards.......... PB7 PB6                    3.3v/5v
  ESP8266 ESP-01........................... GPIO0/D5
  GPIO2/D3               3.3v/5v NodeMCU 1.0, WeMos D1 Mini...............
  GPIO4/D2               GPIO5/D1               3.3v/5v
  ESP32.................................... GPIO21/D21
  GPIO22/D22             3.3v Frameworks & Libraries: ATtiny  Core          -
  https://github.com/SpenceKonde/ATTinyCore ESP32   Core          -
  https://github.com/espressif/arduino-esp32 ESP8266 Core          -
  https://github.com/esp8266/Arduino STM32   Core          -
  https://github.com/rogerclarkmelbourne/Arduino_STM32 GNU GPL license, all text
  above must be included in any redistribution, see link for details  -
...
bool HTU21D::begin(uint8_t sda, uint8_t scl) {
  Wire.begin(sda, scl);
  Wire.setClock(
      100000UL);  // experimental! ESP8266 i2c bus speed:
...
bool HTU21D::begin(void) {
  Wire.begin();
  Wire.setClock(100000UL);  // experimental! AVR i2c bus speed:
                            // 31kHz..400kHz/31000UL..400000UL, default 100000UL
#endif
...
void HTU21D::setResolution(HTU21D_RESOLUTION sensorResolution) {
  ...
  userRegisterData =
      read8(HTU21D_USER_REGISTER_READ);  // reads current user register state
  userRegisterData &= 0x7E;  // clears current resolution bits with 0
  userRegisterData |=
      sensorResolution;  // adds new resolution bits to user register byte

  write8(HTU21D_USER_REGISTER_WRITE,
         userRegisterData);  // writes updeted byte to the user register
...
#endif
  if (Wire.endTransmission(true) != 0)
    return HTU21D_ERROR;  // error handler, collision on the i2c bus

  /* humidity measurement delay */
  switch (_resolution) {
    case HTU21D_RES_RH12_TEMP14:
      delay(29);  // HTU21D - 14..16msec, Si7021 - 10..12msec, SHT21
                  // - 22..29msec
      break;

    case HTU21D_RES_RH11_TEMP11:
      delay(15);  // HTU21D - 7..8msec,   Si7021 - 6..7msec,   SHT21
                  // - 12..15msec
      break;
...
/* read humidity measurement to "wire.h" rxBuffer */
  ...
  Wire.requestFrom(
      HTU21D_ADDRESS, 3,
      true);  // true, stop message after transmission & releas the I2C bus
#endif
  if (Wire.available() != 3)
    return HTU21D_ERROR;  // check rxBuffer & error handler, collision on the
                          // i2c bus
...
/* reads MSB, LSB byte & checksum from "wire.h" rxBuffer */
  ...
  if (checkCRC8(rawHumidity) != checksum)
    return HTU21D_ERROR;  // error handler, checksum verification

  rawHumidity ^= 0x02;  // clear status bits, humidity always returns xxxxxx10
                        // in the LSB field
  humidity = (0.001907 * (float)rawHumidity - 6);

  if (humidity < 0)
    humidity = 0;  // due to RH accuracy, measured value might be slightly less
                   // than 0 or more 100
...
float HTU21D::readTemperature(HTU21D_TEMP_OPERATION_MODE sensorOperationMode) {
  int8_t qntRequest =
      3;  // 3 bytes -> MSB, LSB byte & checksum or 2 bytes -> MSB, LSB byte
  ...
  if (sensorOperationMode != SI70xx_TEMP_READ_AFTER_RH_MEASURMENT) {
    switch (_resolution) {
      case HTU21D_RES_RH12_TEMP14:
        delay(
            85);  // HTU21D - 44..50msec, Si7021 - 7..11msec, SHT21 - 66..85msec
        break;

      case HTU21D_RES_RH10_TEMP13:
        delay(
            43);  // HTU21D - 22..25msec, Si7021 - 4..7msec,  SHT21 - 33..43msec
        break;

      case HTU21D_RES_RH8_TEMP12:
        delay(
            22);  // HTU21D - 11..13msec, Si7021 - 3..4msec,  SHT21 - 17..22msec
        break;

      case HTU21D_RES_RH11_TEMP11:
        delay(11);  // HTU21D - 6..7msec,   Si7021 - 2..3msec,  SHT21
                    // - 9..11msec
        break;
    }
  } else
    qntRequest = 2;  // checksum is not available with
                     // "SI70xx_TEMP_READ_AFTER_RH_MEASURMENT"
...
/* reads MSB, LSB byte & checksum from "wire.h" rxBuffer */
  ...
  if (sensorOperationMode != SI70xx_TEMP_READ_AFTER_RH_MEASURMENT)
    checksum = Wire.read();  // checksum is not available with
                             // "SI70xx_TEMP_READ_AFTER_RH_MEASURMENT"
#else
  ...
  if (sensorOperationMode != SI70xx_TEMP_READ_AFTER_RH_MEASURMENT &&
      checkCRC8(rawTemperature) != checksum)
    return HTU21D_ERROR;  // error handler, checksum verification

  return (0.002681 * (float)rawTemperature -
          46.85);  // temperature always returns xxxxxx00 in the LSB field
}
...
float HTU21D::readCompensatedHumidity(float temperature) {
  ...
  if (temperature == HTU21D_FORCE_READ_TEMP)
    temperature = readTemperature();  // force to read temperature
  ...
  if (temperature > 0 && temperature < 80)
    humidity = humidity +
               (25.0 - temperature) *
                   HTU21D_TEMP_COEFFICIENT;  // apply compensation coefficient
